...
def modern_device_detail(device_id):
    ...
    recent = []
    for s in snaps:
        recent.append({"ts":s.ts.isoformat() if s.ts else None,"lat":s.lat,"lon":s.lon,"speed_kmh":round(float(s.speed_mps or 0)*3.6,1),"source":s.source or ""})
    rows = ''.join(f'<tr><td>{_esc(x["ts"])}</td><td>{_esc(x["lat"])}</td><td>{_esc(x["lon"])}</td><td>{_esc(x["speed_kmh"])} km/h</td><td>{_esc(x["source"])}</td></tr>' for x in recent) or '<tr><td colspan="5"><div class="empty">No snapshots.</div></td></tr>'
    body = f"""
...
<div class="split"><div class="card"><h2>Find a recipient</h2><div class="searchbox"><span class="icon">⌕</span><input id="userq" value="{_esc(request.args.get('q') or '')}" placeholder="Search owner, phone, plate, vehicle or device ID…"></div><div id="results" class="list" style="margin-top:10px;max-height:500px;overflow:auto"></div></div>
...
def modern_traffic_hub():
    role, resp = _require_roles("admin", "police", "gk")
    if resp: return resp
    links=[("admin_traffic","Live traffic"),("admin_roads","Roads"),("admin_speeders","Speeders"),("admin_traffic_zones","Traffic zones")]
    cards=[]
    for ep,label in links:
        try: href=url_for(ep)
        except Exception: continue
        cards.append(f'<a class="list-item" href="{_esc(href)}"><div class="row-between"><strong>{_esc(label)}</strong><span>→</span></div><div class="small-muted">Open operational tool</div></a>')
    body=f'''<div class="page-head"><div><div class="eyebrow">Intelligence</div><h1 class="page-title">Traffic operations</h1><p class="page-sub">Map, roads, speed and traffic-zone tools remain available without loading everything into the main dashboard.</p></div></div><div class="grid grid-2">{''.join(cards)}</div>'''
    return _render("Traffic operations",body,"modern_traffic_hub",role=role)
        ...
        try:
            ...
            try:
                ...
                if _core.OverspeedEvent:
                    evt=(_core.OverspeedEvent.query.filter_by(snapshot_id=state.snapshot_id).order_by(core.OverspeedEvent.ts.desc()).first())
                    if evt:
...
def install_modern_ui(core):
    ...
    globals()["DeviceAlert"]=DeviceAlert
