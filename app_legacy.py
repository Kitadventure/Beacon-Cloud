...
# Legacy compatibility socket configuration; the repaired entrypoint controls production deployment.
_allowed_origins = [x.strip() for x in os.environ.get("SOCKET_ALLOWED_ORIGINS", "").split(",") if x.strip()]
...
def init_db():
    with app.app_context():
        db.create_all()
        # message tables are included here; if the backend is using a fresh DB they are created automatically.
...
def admin_messages():
    ...
    # Load each section defensively so a schema hiccup never crashes the whole page.
    try:
        latest = _latest_snapshots_map()
    except Exception:
        app.logger.exception('messages page: failed to load latest snapshots')
        latest = {}

    try:
        devices = Device.query.filter_by(revoked=False).order_by(Device.created_at.desc()).all()
    except Exception:
        app.logger.exception('messages page: failed to load devices')
        devices = []
    ...
    try:
        ...
        app.logger.exception('messages page: failed to load roads')
        roads = []

    try:
        zones = TrafficZone.query.order_by(TrafficZone.created_at.desc()).all()
    except Exception:
        app.logger.exception('messages page: failed to load zones')
        zones = []

    try:
        recent_messages = BroadcastMessage.query.order_by(BroadcastMessage.created_at.desc()).limit(20).all()
    except Exception:
        app.logger.exception('messages page: failed to load recent messages')
        recent_messages = []
...
def admin_message_search_devices():
    ...
    try:
        ...
        app.logger.exception('message search: failed to load devices')
        return jsonify({'devices': []})

    try:
        latest = _latest_snapshots_map()
    except Exception:
        app.logger.exception('message search: failed to load latest snapshots')
        latest = {}
