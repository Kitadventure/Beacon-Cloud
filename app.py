...
class DeviceAlert(db.Model):
    ...
    kind = db.Column(db.String(64), index=True, nullable=False)
    severity = db.Column(db.String(16), default="info", index=True)
    title = db.Column(db.String(255), nullable=False)
    body = db.Column(db.Text, nullable=False)
    incident_id = db.Column(db.String(36), index=True, nullable=True)
    unique_key = db.Column(db.String(255), index=True, nullable=True)
...
class PlaceCache(db.Model):
    ...
    key = db.Column(db.String(96), primary_key=True)
    lat = db.Column(db.Float, nullable=False)
    lon = db.Column(db.Float, nullable=False)
    place_name = db.Column(db.String(255), nullable=False)
    provider = db.Column(db.String(64), default="nominatim")
...
class LiveVehicleState(db.Model):
    ...
    place_name = db.Column(db.String(255), nullable=True, index=True)
        ...
        if backend == "sqlite":
            ...
            for table, column, sql_type in (("live_vehicle_state", "place_name", "VARCHAR(255)"), ("operational_incident", "place_name", "VARCHAR(255)"), ("road", "one_way", "BOOLEAN"), ("road", "lane_count", "INTEGER")):
                ...
                if column not in cols:
                    db.session.execute(db.text(f"ALTER TABLE {table} ADD COLUMN {column} {sql_type}"))
            db.session.execute(db.text("UPDATE road SET one_way = COALESCE(one_way, 0), lane_count = COALESCE(lane_count, 2)"))
            db.session.commit()
        elif backend == "postgresql":
            db.session.execute(db.text("ALTER TABLE device ADD COLUMN IF NOT EXISTS phone_number VARCHAR(32)"))
            db.session.execute(db.text("ALTER TABLE live_vehicle_state ADD COLUMN IF NOT EXISTS place_name VARCHAR(255)"))
            db.session.execute(db.text("ALTER TABLE operational_incident ADD COLUMN IF NOT EXISTS place_name VARCHAR(255)"))
            db.session.execute(db.text("ALTER TABLE road ADD COLUMN IF NOT EXISTS one_way BOOLEAN"))
...
_place_executor = None
_place_executor_lock = threading.RLock()
_place_last_requested: dict[str, float] = {}
_place_ttl_s = float(os.environ.get("PLACE_CACHE_TTL_S", "900"))
_place_min_move_m = float(os.environ.get("PLACE_MIN_MOVE_M", "100"))
_place_last_http_at = 0.0
_place_http_lock = threading.RLock()
_place_http_min_interval_s = 1.15

def _place_key(lat: float, lon: float) -> str:
    return f"{round(float(lat), 4):.4f},{round(float(lon), 4):.4f}"

def _human_place(address: dict[str, Any], fallback: str = "") -> str:
    order = ("road", "neighbourhood", "suburb", "town", "village", "city", "county", "state", "country")
    vals=[]
    for key in order:
        value=str(address.get(key) or "").strip()
        if value and value not in vals: vals.append(value)
        if len(vals)>=4: break
    return ", ".join(vals) or fallback

def _reverse_geocode(lat: float, lon: float) -> str:
    key=_place_key(lat,lon)
    try:
        cached=PlaceCache.query.filter_by(key=key).first()
        if cached and cached.fetched_at and (datetime.utcnow()-cached.fetched_at).total_seconds() < _place_ttl_s:
            return cached.place_name
    except Exception:
        db.session.rollback()
    # Reverse geocoding is rate-limited globally and sends rounded coordinates.
    global _place_last_http_at
    with _place_http_lock:
        wait = _place_http_min_interval_s - (time.monotonic() - _place_last_http_at)
        if wait > 0:
            time.sleep(wait)
        _place_last_http_at = time.monotonic()
    geocode_lat = round(float(lat), 4)
    geocode_lon = round(float(lon), 4)
    q=urllib.parse.urlencode({"lat":geocode_lat,"lon":geocode_lon,"format":"jsonv2","zoom":18,"addressdetails":1})
    req=urllib.request.Request("https://nominatim.openstreetmap.org/reverse?"+q, headers={"User-Agent":"BeaconCloud/2026.09 road-safety-platform"})
    with urllib.request.urlopen(req, timeout=5) as resp:
        payload=json.loads(resp.read().decode("utf-8"))
    name=_human_place(payload.get("address") or {}, payload.get("display_name") or f"Near {lat:.5f}, {lon:.5f}")[:255]
    try:
        row=PlaceCache.query.filter_by(key=key).first()
        if row is None: row=PlaceCache(key=key,lat=float(lat),lon=float(lon),place_name=name,provider="nominatim")
        else: row.lat=float(lat); row.lon=float(lon); row.place_name=name; row.provider="nominatim"
        row.fetched_at=datetime.utcnow(); db.session.add(row); db.session.commit()
    except Exception:
        db.session.rollback()
    return name

def _schedule_place_lookup(device_id: str, lat: float, lon: float):
    global _place_executor
    now=time.time()
    with _place_executor_lock:
        if now-_place_last_requested.get(device_id,0) < 20: return
        _place_last_requested[device_id]=now
        if _place_executor is None:
            from concurrent.futures import ThreadPoolExecutor
            _place_executor=ThreadPoolExecutor(max_workers=1, thread_name_prefix="beacon-place")
        ...
        try:
            with app.app_context():
                ...
                if state and state.place_name:
                    try:
                        if _haversine_m(state.lat,state.lon,lat,lon) < _place_min_move_m:
                            with _live_cache_lock:
                                ent=_live_cache.get(device_id)
                                if ent: ent["place_name"]=state.place_name
                            return
                    except Exception: pass
                name=_reverse_geocode(lat,lon)
                state=LiveVehicleState.query.filter_by(device_id=device_id).first()
                if state: state.place_name=name; db.session.commit()
                with _live_cache_lock:
                    ent=_live_cache.get(device_id)
                    if ent: ent["place_name"]=name
        except Exception as exc:
            try:
                with app.app_context(): record_system_error(exc,source="place:reverse-geocode",severity="WARNING")
            except Exception: pass
        ...
        for row in live_rows:
            ...
            if row.device_id not in cached_ids or row.updated_at > rows[row.device_id].get("ts", datetime.min):
                rows[row.device_id] = {
                    ...
                    "sequence": row.sequence, "session_id": row.session_id, "source": row.source, "place_name": row.place_name
                }
    except Exception as exc:
...
def _road_profile_for_point(lat: float, lon: float):
    """Return the nearest configured road profile inside its monitoring radius."""
    try:
        roads = Road.query.filter(Road.center_lat.isnot(None), Road.center_lon.isnot(None)).all()
        best = None; best_d = float("inf")
        for road in roads:
            d = _haversine_m(lat, lon, road.center_lat, road.center_lon)
            radius = float(road.radius_m or 50.0)
            if d <= radius and d < best_d:
                best, best_d = road, d
        if best is not None:
            return {"id": best.id, "name": best.name, "one_way": bool(getattr(best, "one_way", False)), "lane_count": int(getattr(best, "lane_count", 2) or 2)}
    except Exception as exc:
        record_system_error(exc, source="risk:road-profile", severity="WARNING")
    return None
...
def compute_nearby_v2(device_id: str, radius_m: float = NEARBY_DEFAULT_RADIUS_M) -> dict[str, Any]:
    ...
    if entry and entry.get("ts") and entry["ts"] >= datetime.utcnow() - timedelta(seconds=LIVE_CACHE_TTL_S):
        ...
        self_snap.place_name = entry.get("place_name")
    if not self_snap:
        state = LiveVehicleState.query.filter_by(device_id=device_id).first()
        if state and state.updated_at and state.updated_at >= datetime.utcnow() - timedelta(seconds=LIVE_CACHE_TTL_S):
            ...
            self_snap.ts = state.ts; self_snap.raw = None; self_snap.place_name = state.place_name
    if not self_snap:
    ...
    if not self_snap:
        ...
        self_snap.ts = entry["ts"]; self_snap.raw = entry.get("raw"); self_snap.place_name = entry.get("place_name")
    ...
    for other_id, entry in live_entries:
        ...
        other.place_name = entry.get("place_name")
        ...
        results.append({
            ...
            "place_name": entry.get("place_name") or getattr(other,"place_name",None),
            "car_name": row.car_name if row else None,
            "car_model": row.car_model if row else None,
            "plate": row.plate if row else None,
        })
    # Multiple same-direction passing candidates require a conservative overlap decision.
    passing = [x for x in results if x.get("direction") == "same" and float(self_snap.speed_mps or 0.0) > float(x.get("speed_mps") or 0.0) + 0.01 and float(x.get("distance_m") or 99999) <= 160]
    road_profile = _road_profile_for_point(self_snap.lat, self_snap.lon)
    if len(passing) >= 2:
        wide_one_way = bool(road_profile and road_profile.get("one_way") and int(road_profile.get("lane_count") or 2) >= 3)
        for item in passing:
            if item.get("decision") == "clear":
                item["decision"] = "caution"
                item["reason"] = "multiple_passing_candidates_overlap_risk" if not wide_one_way else "multiple_passing_candidates_wide_one_way_caution"
                item["confidence"] = round(min(float(item.get("confidence") or 0.0), 0.85), 2)
            item["overtake_candidate"] = True
    ...
    payload = {
        "self": {
            ...
            "lat": self_snap.lat,
            "lon": self_snap.lon,
            "place_name": getattr(self_snap, "place_name", None),
...
def _render_admin_credentials():
    """Read the sole chief-admin credential pair from Render.

    This deployment intentionally uses only ADMIN_NAME + ADMIN_PASSWORD for
    the chief administrator. Public registration remains disabled.
    """
    username = (os.environ.get("ADMIN_NAME") or "").strip()
...
def _record_incident(kind: str, *, device_id=None, plate=None, lat=None, lon=None, severity="medium", confidence=0.0, reason="", evidence=None) -> OperationalIncident:
    ...
    if lat is not None and lon is not None:
        try:
            cached = PlaceCache.query.filter_by(key=_place_key(float(lat), float(lon))).first()
            place_name = cached.place_name if cached else None
        except Exception:
            db.session.rollback()
    if existing:
    ...
    incident = OperationalIncident(type=kind, device_id=device_id, plate=plate, lat=lat, lon=lon,
                                   place_name=place_name, severity=severity, confidence=confidence, reason=reason,
...
def secure_heartbeat():
    ...
    try:
        ...
        db.session.commit()
        _schedule_place_lookup(device_id, lat, lon)
    except Exception as exc:
        db.session.rollback()
...
@app.route("/device/bridge", methods=["GET"])
def device_bridge():
    auth = request.headers.get("Authorization", "")
    header_token = auth.split(" ", 1)[1].strip() if auth.lower().startswith(("token ", "bearer ")) else ""
    device_id=(request.args.get("device_id") or "").strip()
    token=(request.args.get("token") or header_token).strip()
    device=Device.query.filter_by(id=device_id, token=token, revoked=False).first() if device_id and token else None
    if not device:
        return "<html><body>Beacon bridge authorization required.</body></html>", 401
    page = (
        "<!doctype html><meta name='viewport' content='width=device-width,initial-scale=1'>"
        "<title>Beacon bridge</title><script>"
        "async function tick(){try{const r=await fetch('/device/bridge/data',{cache:'no-store',credentials:'same-origin'});"
        "const d=await r.json();if(window.BeaconBridge&&window.BeaconBridge.onPacket)window.BeaconBridge.onPacket(JSON.stringify(d));}"
        "catch(e){if(window.BeaconBridge&&window.BeaconBridge.onError)window.BeaconBridge.onError(String(e));}}"
        "tick();setInterval(tick,5000);</script><body></body>"
    )
    response=Response(page,mimetype="text/html")
    response.set_cookie("beacon_device_id",device.id,httponly=True,secure=request.is_secure,samesite="Lax",max_age=86400)
    response.set_cookie("beacon_device_token",device.token,httponly=True,secure=request.is_secure,samesite="Lax",max_age=86400)
    return response

@app.route("/device/bridge/data", methods=["GET"])
def device_bridge_data():
    auth = request.headers.get("Authorization", "")
    header_token = auth.split(" ", 1)[1].strip() if auth.lower().startswith(("token ", "bearer ")) else ""
    device_id=request.cookies.get("beacon_device_id") or request.args.get("device_id") or ""
    token=request.cookies.get("beacon_device_token") or request.args.get("token") or header_token
    device=Device.query.filter_by(id=device_id, token=token, revoked=False).first() if device_id and token else None
    if not device:
        return _api_error("Device bridge authentication required",401,"DEVICE_AUTH_REQUIRED")
    try:
        ...
        except Exception as exc: record_system_error(exc,source="bridge:messages",severity="WARNING")
        try:
            alerts=DeviceAlert.query.filter(DeviceAlert.device_id==device.id, DeviceAlert.read_at.is_(None), DeviceAlert.kind != "authority_message").order_by(DeviceAlert.created_at.asc()).limit(20).all()
            payload["alerts"]=[{"alert_id":a.id,"type":a.kind,"severity":a.severity,"title":a.title,"body":a.body,"message":a.body,"incident_id":a.incident_id,"created_at":a.created_at.isoformat() if a.created_at else None} for a in alerts]
        except Exception as exc: record_system_error(exc,source="bridge:alerts",severity="WARNING")
        return jsonify(payload)
    except Exception as exc:
        record_system_error(exc,source="bridge:data",severity="ERROR")
        return _api_error("Bridge data unavailable",500,"BRIDGE_DATA_FAILED")

@app.route("/device/alerts/<int:alert_id>/ack", methods=["POST"])
def device_alert_ack_compat(alert_id):
    device=_strict_device_from_request()
    if not device: return _api_error("Missing or invalid device token",401,"DEVICE_AUTH_REQUIRED")
    row=DeviceAlert.query.filter_by(id=alert_id,device_id=device.id).first()
    if not row: return _api_error("Alert not found",404,"ALERT_NOT_FOUND")
    now=datetime.utcnow(); row.read_at=now; row.delivered_at=row.delivered_at or now
    db.session.commit()
